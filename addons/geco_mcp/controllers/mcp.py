...
from odoo.addons.geco_mcp.core.route import mcp_route
from odoo.addons.geco_mcp.tools import common, protocol
from odoo.addons.geco_mcp.tools.content import (
...
class MCPController(http.Controller):
    ...
    def _get_session(self, session_id):
        if session := request.env['geco_mcp.session'].sudo().search([
    ...
    def _claim_notifications(self, session_id, after_id=0):
        table = SQL.identifier('geco_mcp_notification')
    ...
    def _handle_initialize(self, params):
        session = request.env['geco_mcp.session'].sudo().create({
            'user_id': request.env.uid,
            'initialized': False,
        })
    ...
    def _handle_tools_list(self, params):
        return {
            'tools': request.env['geco_mcp.tool'].sudo().get_tools(
                registry='mcp'
            )
        }
    ...
    def _handle_tools_call(self, params):
        ...
        try:
            result, _record_info = retrying(
                partial(
                    request.env['geco_mcp.tool']._call,
                    tool_name,
    ...
    def _handle_resources_read(self, params):
        ...
        try:
            mimetype, raw, name = (
                request.env['geco_mcp.mixin']._resolve_resource_uri(
                    uri
                )
            )
    ...
    def mcp_get(self, **kw):
        ...
        if last_event_id := request.httprequest.headers.get('Last-Event-ID'):
            if resume := request.env['geco_mcp.notification'].search(

...
class TestMcpPrintReport(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'geco_mcp.test_print_report',
            'key': 'geco_mcp.test_print_report',
            'arch': (
                '<t t-name="geco_mcp.test_print_report">'
        ...
        cls.report = cls.env['ir.actions.report'].create({
            ...
            'report_name': 'geco_mcp.test_print_report',
    ...
    def test_print_by_report_name(self):
        result = self._call('print_report', {
            'report_ref': 'geco_mcp.test_print_report',
            'ids': [self.partner.id],
        })
        decoded = base64.b64decode(result['content_base64']).decode()
        self.assertIn('MCP Print Target', decoded)
    ...
    def test_print_by_xmlid_prefix(self):
        ...
        if not xmlid:
            self.env['ir.model.data'].create({
                'name': 'geco_mcp_test_action_print_report',
                'module': 'geco_mcp',
                'model': 'ir.actions.report',
                'res_id': self.report.id,
            })
            xmlid = 'geco_mcp.geco_mcp_test_action_print_report'
        result = self._call('print_report', {
            'report_ref': xmlid,
            'ids': [self.partner.id],
        })
    ...
    def test_print_unknown_xmlid_raises(self):
        with self.assertRaises(UserError):
            self._call('print_report', {
                'report_ref': 'geco_mcp.nonexistent_xmlid',
                'ids': [self.partner.id],
            })

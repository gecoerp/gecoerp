...
from odoo.addons.geco_mcp.controllers import mcp as mcp_controller
from odoo.addons.geco_mcp.core.tool import invalidate_registry_cache, mcp_tool
...
def _mcp_test_ctx_probe(self):
    return {'flag': self.env.context.get('geco_mcp_probe')}
...
class TestMcpDispatch(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.tool_model = cls.env['geco_mcp.tool']
        cls.mixin_cls = type(cls.env['geco_mcp.mixin'])
    ...
    def test_context_override_reaches_db_tool(self):
        tool = self.tool_model.sudo().create({
            ...
            'description': 'Return the probe flag from env.context.',
            'category': 'read',
            'code': "result = {'flag': env.context.get('geco_mcp_probe')}\n",
            'input_schema': json.dumps({'type': 'object', 'properties': {}}),
        })
        try:
            text, _info = self.tool_model._call(
                'mcp_test_ctx_probe_db',
                {'context': {'geco_mcp_probe': 'db_here'}},
                self.env,
            )
    ...
    def test_context_override_does_not_mutate_caller_env(self):
        self.tool_model._call(
            'mcp_test_ctx_probe',
            {'context': {'geco_mcp_probe': 'temp'}},
            self.env,
        )
        self.assertIsNone(self.env.context.get('geco_mcp_probe'))

...
class TestPlayground(common.TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.Key = cls.env['geco_mcp.key']
        cls.Tool = cls.env['geco_mcp.tool']
    ...
    def test_client_action_registered(self):
        action = self.env.ref('geco_mcp.action_mcp_playground')
        self.assertEqual(action.tag, 'geco_mcp.playground')
        self.assertEqual(action.type, 'ir.actions.client')

    def test_menu_positioned_after_audit_log(self):
        menu = self.env.ref('geco_mcp.menu_mcp_playground')
        self.assertEqual(menu.parent_id, self.env.ref('geco_mcp.menu_mcp_root'))
        self.assertEqual(menu.sequence, 35)
        audit = self.env.ref('geco_mcp.menu_mcp_log')

...
from odoo.addons.geco_mcp.tools.schema import to_strict_schema


def _build_method_index(env):
    index = {}
    for anchor in ('geco_mcp.mixin',):
...
def _fetch_db_index(env):
    index = {}
    records = env['geco_mcp.tool'].sudo().search_read(
        [('active', '=', True)],
        fields=[
            'id', 'name', 'description', 'input_schema', 'category', 'registry',
            'is_auto_synced',
        ],
    )
    for record in records:
        ...
        index[record['name']] = {
            ...
            'is_auto_synced': record.get('is_auto_synced', False),
        }
    return index
...
def get_tool_index(env, registry=None):
    ...
    method_index = getattr(env.registry, '_geco_mcp_method_cache', None)
    cached_key = getattr(env.registry, '_geco_mcp_method_cache_key', None)
    stale = (
    ...
    if stale:
        ...
        env.registry._geco_mcp_method_cache = method_index
        env.registry._geco_mcp_method_cache_key = cache_key
...
def invalidate_registry_cache(env):
    if hasattr(env.registry, '_geco_mcp_method_cache'):
        del env.registry._geco_mcp_method_cache
    if hasattr(env.registry, '_geco_mcp_method_cache_key'):
        del env.registry._geco_mcp_method_cache_key

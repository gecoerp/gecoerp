...
Any Odoo addon can register MCP tools by inheriting ``geco_mcp.mixin``
and decorating public methods with ``@mcp_tool``. The geco_mcp scanner
...
.. code-block:: python
    ...
    {
        'name': 'My Sales Tools',
        'depends': ['geco_mcp', 'sale'],
        ...
    }

Step 2 — extend ``geco_mcp.mixin`` and decorate your methods:
...
2. ``geco_mcp.tool._call`` pops any ``context`` key, merges it into
...
**Helpers available on ``geco_mcp.mixin``**
...
Call ``geco_mcp.tool._call`` directly from a ``TransactionCase``:
...
.. code-block:: python
    ...
    class TestMySaleTools(common.TransactionCase):
        ...
        def test_confirm_sale_order(self):
            ...
            text, info = self.env['geco_mcp.tool']._call(

...
from gecoerp import fields as gecoerp_fields, tools, _
from gecoerp.osv import expression
from gecoerp.exceptions import ValidationError
from gecoerp.http import Controller, request, route
from gecoerp.addons.web.controllers.main import WebClient
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': gecoerp_fields.Date.to_string(gecoerp_fields.Date.from_string(date_begin)),
                'date_end': gecoerp_fields.Date.to_string(gecoerp_fields.Date.from_string(date_end)),
                'name': label,

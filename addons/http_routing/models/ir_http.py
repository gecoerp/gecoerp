...
from gecoerp.addons.base.ir.ir_http import RequestUID, ModelConverter
from gecoerp.http import request
from gecoerp.tools import config, ustr, pycompat
...
def is_multilang_url(local_url, langs=None):
    ...
    try:
        ...
        # Force to check method to POST. GECOERP uses methods : ['POST'] and ['GET', 'POST']
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if gecoerp._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            gecoerp._geoip_resolver = GeoIPResolver.open(geofile) or False
        except Exception as e:
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if gecoerp._geoip_resolver and request.httprequest.remote_addr:
                record = gecoerp._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except gecoerp.exceptions.MissingError:

...
import gecoerp
import gecoerp.modules.registry
from gecoerp.api import call_kw, Environment
from gecoerp.modules import get_resource_path
from gecoerp.tools import crop_image, topological_sort, html_escape, pycompat
from gecoerp.tools.translate import _
from gecoerp.tools.misc import str2bool, xlwt, file_open
from gecoerp.tools.safe_eval import safe_eval
from gecoerp import http
from gecoerp.http import content_disposition, dispatch_rpc, request, \
    serialize_exception as _serialize_exception, Response
from gecoerp.exceptions import AccessError, UserError
from gecoerp.models import check_method_name
from gecoerp.service import db
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('gecoerp.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, gecoerp.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = gecoerp.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("gecoerp.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, GECOERP has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = gecoerp.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except gecoerp.exceptions.AccessDenied:
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return gecoerp.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = gecoerp.tools.config.verify_admin_password('admin')
        d['list_db'] = gecoerp.tools.config['list_db']
        d['langs'] = gecoerp.service.db.exp_list_lang()
        d['countries'] = gecoerp.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = gecoerp.service.db.list_db_incompatible(d['databases'])
        except gecoerp.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            gecoerp.service.db.check_super(master_pwd)
            ...
            dump_stream = gecoerp.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=gecoerp.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.gecoerp.com/oauth2/auth?' + werkzeug.url_encode(params)
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if crop and (width or height):
            ...
            if height > 500:
                height = 500
            content = gecoerp.tools.image_resize_image(base64_source=content, size=(width or None, height or None), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = gecoerp.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: gecoerp.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from GECOERP's export data to whatever the

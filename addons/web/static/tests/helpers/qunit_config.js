 ...
 * QUnit Config
 *
 * The GECOERP javascript test framework is based on QUnit (http://qunitjs.com/).
 * This file is necessary to setup Qunit and to prepare its interactions with
 * GECOERP.  It has to be loaded before any tests are defined.
 *
 * Note that it is not an GECOERP module, because we want this code to be executed
 * as soon as possible, not whenever the GECOERP module system feels like it.
 */
 ...
 * this is done outside of gecoerp modules, otherwise the setting would not take
 * effect on time.
 */

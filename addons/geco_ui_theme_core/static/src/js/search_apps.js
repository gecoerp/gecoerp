...
patch(NavBar.prototype, {
    setup() {
        ...
        this._searchMenusSchedule = debounce(this._searchMenus.bind(this), 150);

        this._setupMenuObserver();

        onMounted(() => {
            this._initializeSearchElements();
            if (this._checkMobileView) this._checkMobileView();
        });
                ...
                <a class="o-menu-search-result dropdown-item col-12 ml-auto mr-auto ${index === 0 ? 'active' : ''}"
                   ...
                   role="menuitem">
                    <div class="app-icon d-flex align-items-center">
                        ...
                        <span class="ms-2 text-white">${result.name}</span>
                    </div>
                </a>
            </div>
        `).join('');
    ...
    _renderResultIcon(result) {
        if (!result.webIconData) {
            return '<i class="fa fa-th-large text-warning me-2" style="font-size: 20px;"></i>';
        }
        if (result.webIconData.startsWith('data:image/')) {
            return `<img src="${result.webIconData}"
                        class="search-result-icon"
                        style="width: 24px; height: 24px; border-radius: 6px; margin-right: 8px;"/>`;
        }

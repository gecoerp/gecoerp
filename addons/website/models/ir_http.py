...
from gecoerp.exceptions import QWebException
from gecoerp.tools.safe_eval import safe_eval
from gecoerp.osv.expression import FALSE_DOMAIN, OR

from gecoerp.addons.http_routing.models.ir_http import ModelConverter, _guess_mimetype
from gecoerp.addons.portal.controllers.portal import _build_url_w_params
...
class Http(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if not is_website_request:
            ...
            if isinstance(exception, gecoerp.exceptions.AccessError):
                code = 403

            if isinstance(exception, QWebException):
                values.update(qweb_exception=exception)
                if isinstance(exception.qweb.get('cause'), gecoerp.exceptions.AccessError):
                    code = 403

gecoerp.define('mass_mailing.editor', function (require) {
"use strict";
...
var gecoerp_top = window.top.gecoerp;
...
snippets_editor.Class.include({
    _computeSnippetTemplates: function (html) {
        ...
        $mail_themes_upgrade.on("click", "> a", function (e) {
            ...
            e.preventDefault();
            gecoerp_top[window.callback+"_do_action"]("mass_mailing.action_mass_mailing_configuration");
        });
        ...
        $dropdown.on("click", "li > a", function (e) {
            ...
            // Notify form view
            gecoerp_top[window.callback+"_downup"]($editable_area.addClass("o_dirty").html());
        });
...
gecoerp_top[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window || window.closed) {
        delete gecoerp_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && editor_enable) {
        if (value.indexOf('on_change_model_and_list') !== -1) {
            gecoerp_top[callback+"_downup"](_val);
        }
    }
};

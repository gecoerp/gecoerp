...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return gecoerp.sql_db.db_connect(db_name)
...
def trans_generate(lang, modules, cr):
    ...
    path_list = [(path, True) for path in gecoerp.modules.module.ad_paths]
        ...
        for root, dummy, files in walksymlinks(path):
            ...
            if fnmatch.fnmatch(root, '*/static/src/xml*'):
                for fname in fnmatch.filter(files, '*.xml'):
                    babel_extract_terms(fname, path, root, 'gecoerp.tools.translate:babel_extract_qweb',
                                        extra_comments=[WEB_TRANSLATION_COMMENT])
            if not recursive:
...
def trans_load_data(cr, fileobj, fileformat, lang, lang_name=None, verbose=True, module_name=None, context=None):
    ...
    env = gecoerp.api.Environment(cr, gecoerp.SUPERUSER_ID, context or {})
    Lang = env['res.lang']
    Translation = env['ir.translation']

...
from gecoerp.tools.arabic_reshaper import reshape
from gecoerp.tools.parse_version import parse_version
from gecoerp.tools.misc import file_open
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Gecoerp's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Gecoerp's logo in the banner.
    ...
    gecoerp_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    gecoerp_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        can.setFillColor(gecoerp_color)
        ...
        logo and can.drawImage(
            ImageReader(gecoerp_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class GecoerpPdfFileReader(PdfFileReader):
...
class GecoerpPdfFileWriter(PdfFileWriter):
    ...
    def embed_gecoerp_attachment(self, attachment, subtype=None, afrelationship='/Data'):
        assert attachment, "embed_gecoerp_attachment cannot be called without attachment."

...
from gecoerp.exceptions import ValidationError
from gecoerp.tools.safe_eval import _UNSAFE_ATTRIBUTES
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=gecoerp.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
                  relativedelta=relativedelta,
                  version=gecoerp.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval := node.get('eval'):
            ...
            try:
                ...
                logging.getLogger('gecoerp.tools.convert.init').error(
        ...
        # invoke method
        return gecoerp.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                ...
                groups.append(gecoerp.Command.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(gecoerp.Command.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec, extra_vals=None):
        ...
        for field in rec.iterchildren('field'):
            ...
            if f_search := field.get("search"):
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [gecoerp.Command.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler gecoerp.tools.convert:DEBUG" for complete traceback
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <gecoerp> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['gecoerp', 'data', 'openerp']

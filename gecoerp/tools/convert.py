...
def _eval_xml(self, node, env):
    if node.tag in ('field','value'):
        ...
        return gecoerp.api.call_kw(model, method, args, {})
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def parse(self, de, mode=None):
        roots = ['openerp','data','gecoerp']
        if de.tag not in roots:
            raise Exception("Root xml tag must be <openerp>, <gecoerp> or <data>.")
        for rec in de:
            if rec.tag in roots:
                self.parse(rec, mode)
    ...
    def __init__(self, cr, module, idref, mode, report=None, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.env = gecoerp.api.Environment(cr, SUPERUSER_ID, {})
        self.cr = cr

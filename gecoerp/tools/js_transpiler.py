"""
This code is what let us use ES6-style modules in gecoerp.
Classic GECOERP modules are composed of a top-level :samp:`gecoerp.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@gecoerp-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** gecoerp-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual gecoerp one with minimal changes.
...
from gecoerp.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom gecoerp modules.
    ...
    legacy_gecoerp_define = get_aliased_gecoerp_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_gecoerp_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_gecoerp_define:
        content += legacy_gecoerp_define
    return content
...
def url_to_module_path(url):
    """
    GECOERP modules each have a name. (gecoerp.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_gecoerp_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the gecoerp.define call.
    ...
    """
    return f"""gecoerp.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
GECOERP_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @gecoerp-module                       # '@gecoerp-module' statement
...
def is_gecoerp_module(url, content):
    """
    Detect if the file is a native gecoerp module.
    We look for a comment containing @gecoerp-module.

    :param url:
    :param content: source code
    :return: is this a gecoerp module that need transpilation ?
    """
    result = GECOERP_MODULE_RE.match(content)
...
def get_aliased_gecoerp_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the gecoerp source base being:
    ...
    > gecoerp.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @gecoerp-module */
        // after
        /** @gecoerp-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @gecoerp-module */
        // after
        /** @gecoerp-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = GECOERP_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ngecoerp.define(`%s`, ['%s'], function (require) {
                        return require('%s');

#gecoerp.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.gecoerprc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store GECOERP data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "gecoerp.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="gecoerp.http.rpc.request:DEBUG", help='shortcut for --log-handler=gecoerp.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="gecoerp.http.rpc.response:DEBUG", help='shortcut for --log-handler=gecoerp.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="gecoerp.http:DEBUG", help='shortcut for --log-handler=gecoerp.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="gecoerp.sql_db:DEBUG", help='shortcut for --log-handler=gecoerp.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate GECOERP to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes gecoerp.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            gecoerp.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        gecoerp.netsvc.init_logger()
        gecoerp.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'gecoerp.conf')
        else:
            rcfilepath = os.path.expanduser('~/.gecoerprc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.gecoerprc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferrably '.gecoerprc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('GECOERP_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from gecoerp.modules.module import MANIFEST_NAMES

...
import gecoerp
from gecoerp.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in gecoerp.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [gecoerp.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(gecoerp.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be gecoerp/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = gecoerp.modules.registry.Registry(config['db_name'])
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Gecoerp cloc", lines="Line", other="Other", code="Code")
        s += hr

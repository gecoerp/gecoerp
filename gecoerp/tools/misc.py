...
"""
Miscellaneous tools used by GECOERP.
"""
...
import gecoerp
import gecoerp.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from gecoerp.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from gecoerp.addons.base.models.res_lang import LangData
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if gecoerp.tools.config['db_host']:
        env['PGHOST'] = gecoerp.tools.config['db_host']
    if gecoerp.tools.config['db_port']:
        env['PGPORT'] = str(gecoerp.tools.config['db_port'])
    if gecoerp.tools.config['db_user']:
        env['PGUSER'] = gecoerp.tools.config['db_user']
    if gecoerp.tools.config['db_password']:
        env['PGPASSWORD'] = gecoerp.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    ...
    addons_paths = [*gecoerp.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/gecoerp/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with gecoerp.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with gecoerp.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('gecoerp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('gecoerp.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = gecoerp.fields.Datetime.from_string(value)
            value = gecoerp.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = gecoerp.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = gecoerp.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`gecoerp.models.Model`

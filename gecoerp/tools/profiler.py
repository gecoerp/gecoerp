...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if isinstance(in_self, (gecoerp.sql_db.Cursor, gecoerp.sql_db.TestCursor, gecoerp.sql_db.LazyCursor)):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all gecoerp models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non gecoerp model from the log: [None])
        ...
        .. code-block:: python
          ...
          from gecoerp.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all gecoerp models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/gecoerp/gecoerp/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _gecoerpProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_gecoerpProfile, method)

    wrapper = decorator(_gecoerpProfile, method)
    return wrapper

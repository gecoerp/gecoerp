...
from .result import GecoerpTestResult

_logger = logging.getLogger(__name__)

TEST_MODULE_NAME_PATTERN = re.compile(r'^gecoerp\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if gecoerp.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in gecoerp shell')
        return

    if gecoerp.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from gecoerp.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    gecoerp.tools.config['test_tags'] = test_tags
    gecoerp.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    gecoerp.tools.config['test_enable'] = None
    gecoerp.tools.config['test_tags'] = None

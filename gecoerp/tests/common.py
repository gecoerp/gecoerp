# -*- coding: utf-8 -*-
"""
The module :mod:`gecoerp.tests.common` provides unittest test cases and a few
...
import gecoerp
from gecoerp import api
from gecoerp.exceptions import AccessError
from gecoerp.fields import Command
from gecoerp.http import BadRequest
from gecoerp.modules import module
from gecoerp.modules.registry import Registry
from gecoerp.service import security
from gecoerp.sql_db import BaseCursor, Cursor, TestCursor
from gecoerp.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from gecoerp.tools.mail import single_email_re
from gecoerp.tools.misc import find_in_path, lower_logging
from gecoerp.tools.xml_utils import _validate_xml
...
if gecoerp.cli.COMMAND in ('server', 'start') and not (config['test_enable'] or config['test_file']):
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: gecoerp.tests.common.Form is deprecated, use gecoerp.tests.Form",
...
# The gecoerp library is supposed already configured.
ADDONS_PATH = gecoerp.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = gecoerp.SUPERUSER_ID
...
def get_db_name():
    db = gecoerp.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding GECOERP module name.
    """
    def register(func):
        # register func by gecoerp module name
        if func.__module__.startswith('gecoerp.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most gecoerp operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('gecoerp.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Gecoerp-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.gecoerp.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('GECOERP_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~gecoerp.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(gecoerp.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            gecoerp.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = gecoerp.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('gecoerp.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/gecoerp/addons/" in filename:
                        filename = filename.rsplit("/gecoerp/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: gecoerp.models.BaseModel,
    ...
    def check_test_cursor(self, operation):
        if gecoerp.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {gecoerp.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = gecoerp.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if gecoerp.tools.config['test_tags'] and 'is_query_count' in gecoerp.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | gecoerp.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(gecoerp.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, gecoerp.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        actual_setattr = gecoerp.models.MetaModel.__setattr__
        ...
        def metamodel_setattr(model, key, value):
            ...
            if filename_normalized.endswith('gecoerp/models.py'):
            ...
            actual_setattr(model, key, value)
        cls.classPatch(gecoerp.models.MetaModel, '__setattr__', metamodel_setattr)
        ...
        cls.env = api.Environment(cls.cr, gecoerp.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('gecoerp.addons.base.models.res_users.Users._crypt_context', _crypt_context)
    ...
    def check_attrs(self):
        ...
        modelClassPatches = {
            ...
            for patcher in _patch._active_patches
            if isinstance(patcher.target, gecoerp.models.MetaModel)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f", loglevel=logging.RUNBOT, directory=''):
    ...
    dest = pathlib.Path(gecoerp.tools.config['screenshots']) / get_db_name() / directory
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_gecoerp')
        self.chrome_log_level = logging.RUNBOT

        otc = gecoerp.tools.config
    ...
    def _chrome_start(
        ...
        switches = {
            ...
            '--enable-logging': '',
            '--v': str(int(os.environ.get("GECOERP_BROWSER_LOG_VERBOSITY", "0"))),
            '--no-first-run': '',
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way GECOERP widgets are created they all appear as Class(...)
...
def _find_executable():
    browser_bin_path = os.environ.get('GECOERP_BROWSER_BIN')  # used for testing specific Chrome builds
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if gecoerp.service.server.server is None:
            return None
        return gecoerp.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('gecoerp.service.http.request.')]
        ...
        if request_threads:
            ...
            gecoerp.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        gecoerp.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            gecoerp.http.root.session_store.delete(self.session)

        self.session = session = gecoerp.http.root.session_store.new()
        session.update(
            gecoerp.http.get_default_session(),
            db=get_db_name(),
            # In order to avoid perform a query to each first `url_open`
            # in a test (insert `res.device.log`).
            _trace_disable=True,
        )
        session.context['lang'] = gecoerp.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('gecoerp.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        try:
            ...
            cpu_throttling_os = os.environ.get('GECOERP_BROWSER_CPU_THROTTLING') # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("GECOERP_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"gecoerp.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"gecoerp.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('gecoerp.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if gecoerp.tools.config['test_tags'] and 'is_tour' in gecoerp.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from gecoerp.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in GECOERP test suites

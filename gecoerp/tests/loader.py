...
from .suite import GecoerpSuite
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'gecoerp.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"gecoerp.upgrade.{module}",
        f"gecoerp.addons.{module}.migrations",
        f"gecoerp.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return GecoerpSuite(sorted(tests, key=lambda t: t.test_sequence))
...
def run_suite(suite, global_report=None):
    ...
    results = GecoerpTestResult(global_report=global_report)
    suite(results)

...
GECOERP_TEST_MAX_FAILED_TESTS = max(1, int(os.environ.get('GECOERP_TEST_MAX_FAILED_TESTS', sys.maxsize)))

stats_logger = logging.getLogger('gecoerp.tests.stats')
...
class GecoerpTestResult(object):
    """
    ...
    def _checkShouldStop(self):
        if self.total_errors_count() >= GECOERP_TEST_MAX_FAILED_TESTS:
            ...
            if not global_report.shouldStop:
                _logger.error(
                    ...
                    "Remaining tests will be skipped.", GECOERP_TEST_MAX_FAILED_TESTS)

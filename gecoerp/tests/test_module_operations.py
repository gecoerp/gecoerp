...
import gecoerp
from gecoerp.tools import config, topological_sort, unique
from gecoerp.modules.registry import Registry
from gecoerp.netsvc import init_logger
from gecoerp.tests import standalone_tests
import gecoerp.tests.loader

_logger = logging.getLogger('gecoerp.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of GECOERP modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra GECOERP modules")
...
def test_standalone(args):
    ...
    gecoerp.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        gecoerp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        gecoerp.tools.config['addons_path'] = ','.join([args.addons_path, gecoerp.tools.config['addons_path']])
        if args.data_dir:
            gecoerp.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'gecoerp.modules.loading': {'level': 'CRITICAL'},
            'gecoerp.sql_db': {'level': 'CRITICAL'},
            'gecoerp.models.unlink': {'level': 'WARNING'},
            'gecoerp.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })

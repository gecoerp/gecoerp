...
import gecoerp
from gecoerp.modules import get_module_path, get_modules, initialize_sys_path
...
GECOERP_HELP = """\
Gecoerp CLI, use '{gecoerp_bin} --help' for regular server options.

Available commands:
    {command_list}

Use '{gecoerp_bin} <command> --help' for individual command help."""
...
class Help(Command):
    ...
    def run(self, args):
        ...
        print(GECOERP_HELP.format(  # noqa: T201
            gecoerp_bin=Path(sys.argv[0]).name,
...
def main():
    ...
    if len(args) > 1 and args[0].startswith('--addons-path=') and not args[1].startswith("-"):
        ...
        gecoerp.tools.config._parse_config([args[0]])
        args = args[1:]
    ...
    if len(args) and not args[0].startswith("-"):
        ...
        for module in get_modules():
            if (Path(get_module_path(module)) / 'cli').is_dir():
                try:
                    __import__('gecoerp.addons.' + module)

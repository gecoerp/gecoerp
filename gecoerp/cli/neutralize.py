...
class Neutralize(Command):
    ...
    def run(self, args):
        ...
        opt = gecoerp.tools.config.parse_config(args, setup_logging=True)

        dbname = gecoerp.tools.config['db_name']
        if not dbname:
        ...
        try:
            with gecoerp.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = gecoerp.modules.neutralize.get_installed_modules(cursor)
                    queries = gecoerp.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    gecoerp.modules.neutralize.neutralize_database(cursor)

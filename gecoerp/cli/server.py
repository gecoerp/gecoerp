...
__author__ = gecoerp.release.author
__version__ = gecoerp.release.version

# Also use the `gecoerp` logger for the main script.
_logger = logging.getLogger('gecoerp')

re._MAXCACHE = 4096  # default is 512, a little too small for gecoerp
...
def report_configuration():
    ...
    """
    config = gecoerp.tools.config
    _logger.info("Gecoerp version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', gecoerp.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = gecoerp.tools.config
    if not gecoerp.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = gecoerp.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            gecoerp.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = gecoerp.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = gecoerp.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    gecoerp.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                gecoerp.service.db._create_empty_database(db_name)
            ...
            except gecoerp.service.db.DatabaseExists:
                pass
    ...
    setup_pid_file()
    rc = gecoerp.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the gecoerp server (default command)"""
    def run(self, args):
        gecoerp.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)

...
def report_configuration():
    ...
    _logger.info('addons paths: %s', gecoerp.modules.module.ad_paths)
    ...
    with open(config["translate_out"], "wb") as buf:
        ...
        with gecoerp.api.Environment.manage():
            with registry.cursor() as cr:
                gecoerp.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with gecoerp.api.Environment.manage():
        with registry.cursor() as cr:
            gecoerp.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    gecoerp.tools.config.parse_config(args)
    check_postgres_user()

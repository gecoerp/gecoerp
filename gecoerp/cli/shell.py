...
class Shell(Command):
    """Start gecoerp in an interactive shell"""
    ...
    def init(self, args):
        ...
        gecoerp.cli.server.report_configuration()
        gecoerp.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = gecoerp.api.Environment(cr, uid, {})['res.users'].context_get()
                env = gecoerp.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
                ...
                # from gecoerp.tests.shell.run_tests if the user hasn't done anything.
                cr.rollback()

...
* addon: the gecoerp addon in which the file is;
...
try:
    ...
    from . import Command
    from gecoerp import release
    from gecoerp.modules import initialize_sys_path
    from gecoerp.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via gecoerp-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing gecoerp
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /gecoerp/upgrade_code """
    name = 'upgrade_code'
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            ...
            description=__doc__.replace('/gecoerp/upgrade_code', str(UPGRADE)),
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = gecoerp.addons.__path__
        else:

...
from gecoerp.tools import SQL
from gecoerp.tools.misc import exec_pg_environ, find_pg_tool
...
def _check_faketime_mode(db_name):
    if os.getenv('GECOERP_FAKETIME_TEST_MODE') and db_name in gecoerp.tools.config['db_name'].split(','):
        try:
            db = gecoerp.sql_db.db_connect(db_name)
...
def _create_empty_database(name):
    ...
    try:
        db = gecoerp.sql_db.db_connect(name)
        with db.cursor() as cr:
            ...
            if gecoerp.tools.config['unaccent']:
                ...
                # But in the case of GECOERP, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    with registry.cursor() as cr:
        ...
        if neutralize_database:
            gecoerp.modules.neutralize.neutralize_database(cr)
...
def exp_change_admin_password(new_password):
    ...
    gecoerp.tools.config.save(['admin_passwd'])
    return True
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if gecoerp.tools.sql.table_exists(cr, 'ir_module_module'):
...
def exp_list_lang():
    return gecoerp.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(gecoerp.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()

...
import gecoerp
from gecoerp import SUPERUSER_ID
from gecoerp.exceptions import AccessDenied
import gecoerp.release
import gecoerp.sql_db
import gecoerp.tools
from gecoerp.sql_db import db_connect
from gecoerp.release import version_info
from gecoerp.tools import pycompat
...
def check_super(passwd):
    if passwd and gecoerp.tools.config.verify_admin_password(passwd):
        return True
    raise gecoerp.exceptions.AccessDenied()

# This should be moved to gecoerp.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None):
    try:
        db = gecoerp.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            gecoerp.modules.db.initialize(cr)
            gecoerp.tools.config['load_language'] = lang
            cr.commit()

        registry = gecoerp.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = gecoerp.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = gecoerp.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = gecoerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = gecoerp.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    gecoerp.sql_db.close_db(db_original_name)
    db = gecoerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = gecoerp.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = gecoerp.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = gecoerp.tools.config.filestore(db_original_name)
    to_fs = gecoerp.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    gecoerp.modules.registry.Registry.delete(db_name)
    gecoerp.sql_db.close_db(db_name)
    ...
    fs = gecoerp.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'gecoerp_dump': '1',
        'db_name': cr.dbname,
        'version': gecoerp.release.version,
        'version_info': gecoerp.release.version_info,
        'major_version': gecoerp.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with gecoerp.tools.osutil.tempdir() as dump_dir:
            filestore = gecoerp.tools.config.filestore(db_name)
            ...
            gecoerp.tools.exec_pg_command(*cmd)
            if stream:
                gecoerp.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                gecoerp.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = gecoerp.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with gecoerp.tools.osutil.tempdir() as dump_dir:
        ...
        if gecoerp.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = gecoerp.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = gecoerp.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if gecoerp.tools.config['unaccent']:
                try:
                    with cr.savepoint():
...
def exp_rename(old_name, new_name):
    gecoerp.modules.registry.Registry.delete(old_name)
    gecoerp.sql_db.close_db(old_name)
    ...
    old_fs = gecoerp.tools.config.filestore(old_name)
    new_fs = gecoerp.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    gecoerp.tools.config.set_admin_password(new_password)
    gecoerp.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        gecoerp.tools.config['update']['base'] = True
        gecoerp.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@gecoerp.tools.mute_logger('gecoerp.sql_db')
...
def list_dbs(force=False):
    if not gecoerp.tools.config['list_db'] and not force:
        raise gecoerp.exceptions.AccessDenied()

    if not gecoerp.tools.config['dbfilter'] and gecoerp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, GECOERP will not
        ...
        res = sorted(db.strip() for db in gecoerp.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            db_user = gecoerp.tools.config["db_user"]
            if not db_user and os.name == 'posix':
                import pwd
                db_user = pwd.getpwuid(os.getuid())[0]
            if not db_user:
                cr.execute("select usename from pg_user where usesysid=(select datdba from pg_database where datname=%s)", (gecoerp.tools.config["db_name"],))
                res = cr.fetchone()
            ...
            res = [gecoerp.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
            res = []
    res.sort()
    return res
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of GECOERP
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if gecoerp.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        gecoerp.sql_db.close_db(database_name)
...
def exp_list_lang():
    return gecoerp.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(gecoerp.tools.config['root_path'], 'addons/base/res/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return gecoerp.release.version

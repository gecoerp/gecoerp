...
import gecoerp
from gecoerp.modules.module import run_unit_tests, runs_post_install
from gecoerp.modules.registry import Registry
from gecoerp.release import nt_service_name
from gecoerp.tools import config
from gecoerp.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in gecoerp.modules.module.ad_paths:
            ...
            if not event.is_directory:
                ...
                if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
                    try:
                        ...
                        if not getattr(gecoerp, 'phoenix', False):
...
class ThreadedServer(CommonServer):
    ...
    def signal_handler(self, sig, frame):
        if sig in [signal.SIGINT, signal.SIGTERM]:
            ...
            # restart on kill -HUP
            gecoerp.phoenix = True
            self.quit_signals_received += 1
            # interrupt run() to start shutdown
            raise KeyboardInterrupt()

    def cron_thread(self, number):
        from gecoerp.addons.base.ir.ir_cron import ir_cron
        while True:
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="gecoerp.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        gecoerp.modules.registry.Registry.delete_all()
        logging.shutdown()
...
class WorkerCron(Worker):
    ...
    def process_work(self):
        rpc_request = logging.getLogger('gecoerp.netsvc.rpc.request')
...
def load_server_wide_modules():
    for m in gecoerp.conf.server_wide_modules:
        try:
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        gecoerp.tools.convert_yaml_import(cr, 'base', open(test_file, 'rb'), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in list(sys.modules.items()):
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = gecoerp.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_file']:
                ...
                with gecoerp.api.Environment.manage():
            ...
            if config['test_enable']:
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, gecoerp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    ...
    gecoerp.service.wsgi_server._patch_xmlrpc_marshaller()

    if gecoerp.evented:
        server = GeventServer(gecoerp.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(gecoerp.service.wsgi_server.application)
        ...
        server = ThreadedServer(gecoerp.service.wsgi_server.application)
    ...
    if getattr(gecoerp, 'phoenix', False):
        if watcher:
            watcher.stop()
        _reexec()

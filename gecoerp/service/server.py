...
import gecoerp
from gecoerp.modules import get_modules
from gecoerp.modules.registry import Registry
from gecoerp.release import nt_service_name
from gecoerp.tools import config
from gecoerp.tools.cache import log_ormcache_stats
from gecoerp.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if gecoerp.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    ...
    def setup(self):
        ...
        me.name = 'gecoerp.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The GECOERP_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("GECOERP_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in gecoerp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="gecoerp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from gecoerp.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = gecoerp.modules.registry.Registry.registries
        ...
        while True:
            conn = gecoerp.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(gecoerp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="gecoerp.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from gecoerp.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('gecoerp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(gecoerp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The GECOERP_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("GECOERP_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = gecoerp.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from gecoerp.addons import base
    ...
    def start(self):
        ...
        if registries_size := os.environ.get('GECOERP_REGISTRY_LRU_SIZE_CRON'):
            Registry.registries.count = int(registries_size)

        dbconn = gecoerp.sql_db.db_connect('postgres')
...
def _reexec(updated_modules=None):
    ...
    if gecoerp.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from gecoerp.tests import loader  # noqa: PLC0415
    from gecoerp.tests.suite import GecoerpSuite  # noqa: PLC0415
...
def preload_registries(dbnames):
    ...
    dbnames = dbnames or []
    rc = 0
    registries_size = int(os.environ.get('GECOERP_REGISTRY_LRU_SIZE') or 0)
            ...
            if config['test_enable']:
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = gecoerp.api.Environment(cr, gecoerp.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             gecoerp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the gecoerp http server and cron processor.
    """
    global server
    ...
    if gecoerp.evented:
        server = GeventServer(gecoerp.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(gecoerp.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which GECOERP is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(gecoerp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not gecoerp.evented:
        if inotify:

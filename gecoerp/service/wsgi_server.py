...
# User code must use the exceptions defined in ``gecoerp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, gecoerp.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, gecoerp.tools.ustr(e.name))
    ...
    elif isinstance(e, gecoerp.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = gecoerp.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, gecoerp.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(gecoerp.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(gecoerp.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, gecoerp is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(gecoerp.tools.ustr(value), write)
...
def wsgi_xmlrpc(environ, start_response):
    ...
    if environ['REQUEST_METHOD'] == 'POST' and environ['PATH_INFO'].startswith('/xmlrpc/'):
        ...
        try:
            result = gecoerp.http.dispatch_rpc(service, method, params)
...
def application_unproxied(environ, start_response):
    ...
    # gecoerp.service.web_services.objects_proxy.dispatch().
    ...
    with gecoerp.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, gecoerp.http.root]:

...
import gecoerp
from gecoerp.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from gecoerp.sql_db import TestCursor
from gecoerp.tools import (
    config,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from gecoerp.tools.func import locked
from gecoerp.tools.lru import LRU
from gecoerp.tools.misc import Collector, format_frame
from gecoerp.tools.version_tag_reset import reset_classes_tp_versions_used

if typing.TYPE_CHECKING:
    from gecoerp.models import BaseModel


_logger = logging.getLogger(__name__)
_schema = logging.getLogger('gecoerp.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            from gecoerp.http import borrow_request  # noqa: PLC0415
            # This should be a method on Registry
            try:
                with borrow_request():
                    gecoerp.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                gecoerp.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from gecoerp.tests.result import GecoerpTestResult  # noqa: PLC0415
            self._assertion_report = GecoerpTestResult()
        else:
        ...
        self._db = gecoerp.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = gecoerp.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = gecoerp.modules.db.has_unaccent(cr)
            self.has_trigram = gecoerp.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = gecoerp.api.Environment(cr, SUPERUSER_ID, context)
    ...
    def cursor(self, /, readonly=False):
        ...
        if self.test_cr is not None:
            ...
            return TestCursor(self.test_cr, self.test_lock, readonly and self.test_readonly_enabled, current_test=gecoerp.modules.module.current_test)

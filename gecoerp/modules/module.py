...
from gecoerp import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and gecoerp.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('gecoerp.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        gecoerp_name = re.sub(r'^openerp.addons.(\w+)$', r'gecoerp.addons.\g<1>', name)
        openerp_name = re.sub(r'^gecoerp.addons.(\w+)$', r'openerp.addons.\g<1>', gecoerp_name)

        assert gecoerp_name not in sys.modules
        ...
        # create empty gecoerp.addons.* module, set name
        new_mod = types.ModuleType(gecoerp_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "GECOERP addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = gecoerp_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[gecoerp_name] = sys.modules[openerp_name] = new_mod
        ...
        setattr(gecoerp.addons, addon_name, new_mod)
...
class GECOERPHook(object):
    """ Makes gecoerp package also available as openerp """
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'gecoerp\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import gecoerp.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add gecoerp.addons.__path__
    for ad in __import__('gecoerp.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, GECOERPHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = gecoerp.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('gecoerp.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['gecoerp.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='gecoerp.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, gecoerp.sql_db.sql_counter - t0_sql)

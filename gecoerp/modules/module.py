...
import gecoerp
import gecoerp.tools as tools
import gecoerp.release as release
from gecoerp.tools.misc import file_path
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `gecoerp.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^gecoerp\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("gecoerp.addons.base.maintenance.migrations", "gecoerp.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``gecoerp.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook gecoerp.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in gecoerp.addons.__path__:
        gecoerp.addons.__path__.append(dd)

    # hook gecoerp.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in gecoerp.addons.__path__:
            gecoerp.addons.__path__.append(ad)

    # hook gecoerp.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in gecoerp.addons.__path__ and os.path.isdir(base_path):
        gecoerp.addons.__path__.append(base_path)

    # hook gecoerp.upgrade on upgrade-path
    from gecoerp import upgrade
    ...
    # create decrecated module alias from gecoerp.addons.base.maintenance.migrations to gecoerp.upgrade
    spec = importlib.machinery.ModuleSpec("gecoerp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["gecoerp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["gecoerp.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to gecoerp and "crm"-like to gecoerp.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in gecoerp.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(gecoerp.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'gecoerp.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in gecoerp.addons.__path__:

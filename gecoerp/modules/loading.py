...
import gecoerp
import gecoerp.modules.db
import gecoerp.modules.graph
import gecoerp.modules.migration
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = gecoerp.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = gecoerp.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = gecoerp.sql_db.sql_counter
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), gecoerp.sql_db.sql_counter
                test_results = loader.run_suite(suite, global_report=report)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = gecoerp.sql_db.sql_counter - tests_q0
        ...
        extra_queries = gecoerp.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   gecoerp.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
    ...
    with registry.cursor() as cr:
        ...
        if update_module and tools.config['update']:
            for pyfile in tools.config['pre_upgrade_scripts'].split(','):
                gecoerp.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')

        if update_module and gecoerp.tools.sql.table_exists(cr, 'ir_model_fields'):
            ...
            if gecoerp.tools.sql.column_exists(cr, 'ir_model_fields', 'company_dependent'):

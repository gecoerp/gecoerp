...
def is_initialized(cr):
    ...
    """
    return gecoerp.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = gecoerp.modules.get_module_resource('base', 'base.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)
    base_sql_file = gecoerp.tools.misc.file_open(f)
    try:
    ...
    for i in gecoerp.modules.get_modules():
        ...
        info = gecoerp.modules.load_information_from_description_file(i)

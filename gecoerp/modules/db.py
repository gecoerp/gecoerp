...
def is_initialized(cr):
    ...
    """
    return gecoerp.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    try:
        f = gecoerp.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with gecoerp.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in gecoerp.modules.get_modules():
        mod_path = gecoerp.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = gecoerp.modules.get_manifest(i)
    ...
    from gecoerp.tools import config  # noqa: PLC0415
    while True:
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by GECOERP.

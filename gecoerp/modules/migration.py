...
VERSION_RE = re.compile(
    r"""^
        # Optional prefix with GECOERP version
        ((
            6\.1|
        ...
        # After GECOERP version we allow precisely 2 or 3 parts
...
class MigrationManager(object):
    ...
    def _get_files(self):
        def _get_upgrade_path(pkg):
            for path in gecoerp.upgrade.__path__:
    ...
    def migrate_module(self, pkg, stage):
        ...
        def compare(version):
            ...
            if majorless_version:
                # We should not re-execute major-less scripts when upgrading to new GECOERP version
...
def exec_script(cr, installed_version, pyfile, addon, stage, version=None):
    ...
    if ext.lower() != '.py':
        return
    try:
        mod = load_script(pyfile, f"gecoerp.upgrade.{addon}.{version}.{name}")

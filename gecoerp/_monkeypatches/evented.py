...
It will initialize the `gecoerp.evented` variable.
"""
import gecoerp
import sys

gecoerp.evented = False


def patch_evented():
    if gecoerp.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in gecoerp/debian/copyright)
        while 1:
            state = conn.poll()

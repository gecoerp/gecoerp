...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``gecoerp.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``gecoerp.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['gecoerp', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    GECOERP models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = gecoerp.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or gecoerp.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from gecoerp.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~gecoerp.fields.Date` and
              :class:`~gecoerp.fields.Datetime` fields use strings as values
        ...
        * .. _openerp/models/relationals/format:
          ...
          :class:`~gecoerp.fields.One2many` and
          :class:`~gecoerp.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~gecoerp.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~gecoerp.fields.One2many`.
          ``(5, _, _)``

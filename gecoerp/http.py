...
# *broken*
NO_POSTMORTEM = (gecoerp.osv.orm.except_orm,
                 gecoerp.exceptions.AccessError,
                 gecoerp.exceptions.ValidationError,
                 gecoerp.exceptions.MissingError,
                 gecoerp.exceptions.AccessDenied,
                 gecoerp.exceptions.Warning,
                 gecoerp.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                gecoerp.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = gecoerp.service.common.dispatch
        elif service_name == 'db':
            dispatch = gecoerp.service.db.dispatch
        elif service_name == 'object':
            dispatch = gecoerp.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                gecoerp.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                gecoerp.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except gecoerp.exceptions.DeferredException as e:
        _logger.exception(gecoerp.tools.exception_to_unicode(e))
        gecoerp.tools.debugger.post_mortem(gecoerp.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(gecoerp.tools.exception_to_unicode(e))
        gecoerp.tools.debugger.post_mortem(gecoerp.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all GECOERP Web request types, mostly deals with
    ...
    @property
    def cr(self):
        """ :class:`~gecoerp.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~gecoerp.api.Environment` bound to current request. """
        if self._env is None:
            self._env = gecoerp.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            gecoerp.tools.debugger.post_mortem(
                gecoerp.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return gecoerp.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        GECOERP implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <gecoerp.http.WebRequest.csrf_token`, the
          :data:`~gecoerp.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          GECOERP) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (gecoerp.exceptions.Warning, SessionExpiredException,
                                          gecoerp.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "GECOERP Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "GECOERP Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "GECOERP Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, gecoerp.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, gecoerp.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, gecoerp.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, gecoerp.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, gecoerp.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, gecoerp.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, gecoerp.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, gecoerp.exceptions.except_orm):
...
GECOERP URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.gecoerp.com/documentation/11.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through GECOERP via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["gecoerp", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('gecoerp.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def check_security(self):
        ...
        env = gecoerp.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or gecoerp.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = gecoerp.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        return routing_map([''] + gecoerp.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        # The GECOERP_PRELOAD_ADDONS environment variable is available for version 11.0 only.
        ...
        # activation in such case ` GECOERP_PRELOAD_ADDONS=no ` should be used.
        # Note: GECOERP versions > 11.0 does not preload addons anymore.
        preload_addons = os.environ.get('GECOERP_PRELOAD_ADDONS', 'yes') == 'yes'

        # TODO should we move this to ir.http so that only configured modules are served ?
        statics = {}
        for addons_path in gecoerp.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    ...
                    if manifest_path and os.path.isdir(path_static):
                        ...
                        m = None
                        if 'gecoerp.addons' in sys.modules and preload_addons:
                            m = __import__('gecoerp.addons.' + module)
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        gecoerp.registry(db).check_signaling()
                        with gecoerp.tools.mute_logger('gecoerp.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = gecoerp.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if gecoerp.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = gecoerp.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif gecoerp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, GECOERP will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in gecoerp.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = gecoerp.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = gecoerp.tools.ustr(filename)

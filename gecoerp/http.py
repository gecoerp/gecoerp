...
r"""\
Gecoerp HTTP layer / WSGI application
...
:class:`~gecoerp.http.Controller`: class and its companion the
:func:`~gecoerp.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an GECOERP http request. The GECOERP http request is
...
HTTP methods). See
https://www.gecoerp.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~gecoerp.service.db.list_dbs`:
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': gecoerp.service.common.dispatch,
        'db': gecoerp.service.db.dispatch,
        'object': gecoerp.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~gecoerp.tools.file_path`
        :param filter_ext: See :func:`~gecoerp.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`gecoerp.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~gecoerp.http.Controller` can use the :func:`~gecoerp.http.route`:
    ...
    :func:`~gecoerp.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(gecoerp.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['gecoerp', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~gecoerp.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['gecoerp', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are GECOERP addons.
        """
        # Controllers defined outside of gecoerp addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of gecoerp addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = gecoerp.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = gecoerp.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~gecoerp.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> gecoerp_ip = socket.gethostbyname('gecoerp.com')
        >>> GeoIP(gecoerp_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~gecoerp.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # gecoerp.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~gecoerp.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~gecoerp.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = gecoerp.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to gecoerp.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~gecoerp.http.Response.load` method for the compatible
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in gecoerp.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + gecoerp.conf.server_wide_modules, nodb_only=True):
    ...
    def __call__(self, environ, start_response):
        ...
        if gecoerp.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/gecoerp/')
                            or httprequest.path in (
                                '/gecoerp', '/web', '/web/login', '/test_http/ensure_db',
                            )):

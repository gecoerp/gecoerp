...
MAX_IDLE_TIMEOUT = int(os.getenv("GECOERP_DB_MAX_IDLE_TIMEOUT", "600"))
...
class Cursor(BaseCursor):
    ...
    def __init__(self, pool, dbname, dsn):
        ...
        self.cache = {}
        self._now = None
        if os.getenv('GECOERP_FAKETIME_TEST_MODE') and self.dbname in tools.config['db_name'].split(','):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # GECOERP Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri, readonly=False):
    ...
    :rtype: (str, dict)
    """
    if 'GECOERP_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['GECOERP_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
        app_name = "gecoerp-%d" % os.getpid()
...
def db_connect(to, allow_uri=False, readonly=False):
    ...
    maxconn = gecoerp.evented and tools.config['db_maxconn_gevent'] or tools.config['db_maxconn']
...
def close_db(db_name):
    """ You might want to call gecoerp.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:

...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "gecoerp.netsvc.log is deprecated starting GECOERP 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self, log_db=None):
        ...
        if self._log_db != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('gecoerp.sql_db'), sql_db.db_connect(self._log_db, allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('gecoerp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='gecoerp.tools.image')
    ...
    if conf:
        ...
        if not conf.get('keep_gecoerp_default', False):
            return
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("GECOERP_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'gecoerp.http.rpc.request:INFO',
    'gecoerp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['gecoerp:DEBUG', 'gecoerp.sql_db:INFO', 'gecoerp.http.rpc:DEBUG'],
    'debug_rpc': ['gecoerp:DEBUG', 'gecoerp.sql_db:INFO', 'gecoerp.http.rpc.request:DEBUG'],
    'debug': ['gecoerp:DEBUG', 'gecoerp.sql_db:INFO'],
    'debug_sql': ['gecoerp.sql_db:DEBUG'],
    'info': [],
    'runbot': ['gecoerp:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['gecoerp:WARNING', 'werkzeug:WARNING'],
    'error': ['gecoerp:ERROR', 'werkzeug:ERROR'],
    'critical': ['gecoerp:CRITICAL', 'werkzeug:CRITICAL'],
}

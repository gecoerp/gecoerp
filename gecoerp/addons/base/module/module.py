...
import gecoerp
from gecoerp import api, fields, models, modules, tools, _
from gecoerp.exceptions import AccessDenied, UserError
from gecoerp.tools.parse_version import parse_version
from gecoerp.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'GECOERP Enterprise Edition License v1.0'),
        ('OPL-1', 'GECOERP Proprietary License v1.0'),
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = gecoerp.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/gecoerp/addons directory to the new "gecoerp" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'gecoerp' subdirectory ?
                ...
                _logger.info('Copy downloaded module `gecoerp` to `%s`', server_dir)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                gecoerp.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.gecoerp.com/apps')

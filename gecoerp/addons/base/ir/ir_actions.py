...
from gecoerp.exceptions import MissingError, UserError, ValidationError, AccessError
from gecoerp.tools.safe_eval import safe_eval, test_python_expr
from gecoerp.tools import pycompat, wrap_module
...
class IrActionsServer(models.Model):
    ...
    Since GECOERP 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: GECOERP Environment on which the action is triggered
#  - model: GECOERP Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': gecoerp.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers

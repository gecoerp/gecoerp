...
from gecoerp.exceptions import UserError, AccessError
from gecoerp.tools.safe_eval import safe_eval
from gecoerp.tools.misc import find_in_path
...
# here to init the T1 fonts cache at the start-up of UNO, GECOERP.so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at GECOERP start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start GECOERP with at least two workers to print a pdf version of the reports.')

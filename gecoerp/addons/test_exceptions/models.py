...
import gecoerp.exceptions
import gecoerp.osv.osv
from gecoerp import models, api
...
class m(models.Model):
    ...
    def generate_except_osv(self):
        ...
        raise gecoerp.osv.osv.except_osv('title', 'description')

    @api.multi
    def generate_except_orm(self):
        # title is ignored in the new (6.1) exceptions
        raise gecoerp.exceptions.except_orm('title', 'description')

    @api.multi
    def generate_warning(self):
        raise gecoerp.exceptions.Warning('description')

    @api.multi
    def generate_redirect_warning(self):
        action = self.env.ref('test_exceptions.action_test_exceptions')
        raise gecoerp.exceptions.RedirectWarning('description', action.id, 'Go to the redirection')
    ...
    def generate_access_error(self):
        raise gecoerp.exceptions.AccessError('description')
    ...
    def generate_user_error(self):
        raise gecoerp.exceptions.UserError('description')

    @api.multi
    def generate_missing_error(self):
        raise gecoerp.exceptions.MissingError('description')

    @api.multi
    def generate_validation_error(self):
        raise gecoerp.exceptions.ValidationError('description')
